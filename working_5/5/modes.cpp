...
ModeOutput buildAnalogScope(const std::vector<float>& left, const std::vector<float>& right, const VisState& state) {
    ...
    // P31 phosphor: subdued green
    constexpr float pr = 0.08f, pg = 0.5f, pb = 0.06f;
    ...
    // For particle modes, use ALL samples for maximum particle count
    const size_t particleSamples = (isScatter || isBoth)
        ? std::min({left.size(), right.size(), size_t(4096)})
        : 0;
    // For trace modes, use max resolution
    const size_t traceSamples = (isTrace || isBoth)
        ? std::min({left.size(), right.size(), size_t(4096)})
        : 0;

    // --- Sweep position (Trace and Both) ---
    size_t sweepPos = 0;
    if (traceSamples > 0) {
        const float sweepSpeed = std::max(2.0f, state.waveSpeed);
        const float sweepTime = static_cast<float>(glfwGetTime()) * sweepSpeed;
        const float t = std::fmod(sweepTime, 1.0f);
        sweepPos = static_cast<size_t>(t * static_cast<float>(traceSamples));
    }

    // --- Trace lines (for Trace and Both) — fast sweep, high res, phosphor decay ---
    if (traceSamples > 0 && sweepPos > 0) {
        const std::vector<float> dsLeft = downsample(left, traceSamples);
        const std::vector<float> dsRight = downsample(right, traceSamples);

        out.lineVertices.reserve(sweepPos * 6);
        constexpr float teleportThreshold = 0.35f;
        constexpr float decayRate = 6.0f;
        GLsizei segStart = 0;
        out.linePrimitive = GL_LINE_STRIP;
        out.linePoints    = false;

        Pt prev{0,0};
        for (size_t i = 0; i < sweepPos; ++i) {
            const Pt p = makePt(dsLeft[i], dsRight[i]);
            if (i > 0) {
                const float dx = p.x - prev.x;
                const float dy = p.y - prev.y;
                if (std::sqrt(dx * dx + dy * dy) >= teleportThreshold) {
                    if (static_cast<GLsizei>(out.lineVertices.size() / 6) > segStart) {
                        out.lineSegments.push_back({segStart, static_cast<GLsizei>(out.lineVertices.size() / 6) - segStart});
                    }
                    segStart = static_cast<GLsizei>(out.lineVertices.size() / 6);
                }
            }
            // Phosphor decay: points near the beam head are brightest
            const float age = static_cast<float>(sweepPos - i) / static_cast<float>(sweepPos);
            const float brightness = std::exp(-age * decayRate) * 0.95f + 0.05f;
            pushVertex(out.lineVertices, p.x, p.y, pr * brightness, pg * brightness, pb * brightness, 1.0f);
            prev = p;
        }
        if (static_cast<GLsizei>(out.lineVertices.size() / 6) > segStart) {
            out.lineSegments.push_back({segStart, static_cast<GLsizei>(out.lineVertices.size() / 6) - segStart});
    ...
    // --- Exponential particles (for Scatter and Both) ---
    if (particleSamples > 0) {
        const size_t particleCount = isBoth ? std::min(sweepPos, particleSamples) : particleSamples;
        if (particleCount > 0) {
            ...
            constexpr float pointSize = 0.020f;
            const float s = pointSize;
            out.fillVertices.reserve(particleCount * 6 * 6);

            for (size_t i = 0; i < particleCount; ++i) {
                const Pt p = makePt(dsLeft[i], dsRight[i]);
                pushVertex(out.fillVertices, p.x - s, p.y - s, pr, pg, pb, 1.0f);
                pushVertex(out.fillVertices, p.x + s, p.y - s, pr, pg, pb, 1.0f);
                pushVertex(out.fillVertices, p.x + s, p.y + s, pr, pg, pb, 1.0f);
                pushVertex(out.fillVertices, p.x - s, p.y - s, pr, pg, pb, 1.0f);
                pushVertex(out.fillVertices, p.x + s, p.y + s, pr, pg, pb, 1.0f);

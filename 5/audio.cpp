...
void AudioCapture::pushSamples2(const float* samples, size_t count) {
    std::lock_guard<std::mutex> lock(m_mutex);
    const float sign = (m_mode == CaptureMode::Reduction) ? -1.0f : 1.0f;
    for (size_t i = 0; i < count; ++i) {
        // Mix secondary device samples additively/subtractively into ring buffer
        size_t idx = (m_writeIndex + m_ringSize - count + i) % m_ringSize;
        m_ringBuffer[idx] += samples[i] * sign;
    }
}

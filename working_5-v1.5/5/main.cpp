...
const char* modeName(VisMode mode) {
    switch (mode) {
        ...
        case VisMode::ParticleField:          return "10: Particle Field";
...
int main(int argc, char* argv[]) {
    ...
    auto parseConfigFile = [&](const std::string& path) {
        ...
        while (std::getline(file, line)) {
            ...
            else if (key.size() > 14 && key.substr(0, 14) == "gradientcolor" && v) {
                float r, g, b;
    ...
    auto writeConfigFile = [](const std::string& path, const VisState& state, VisMode mode) {
        ...
        auto capLabel = [](InputMode m) -> const char* {
            switch (m) { case InputMode::Microphone: return "mic"; case InputMode::SystemAudio: return "system"; case InputMode::Both: return "both"; } return "mic";
        };
    ...
    auto loadLiveConfigFile = [](const std::string& path, VisState& state, VisMode& mode) {
        ...
        while (std::getline(file, line)) {
            ...
            if (key == "mode") { mode = static_cast<VisMode>(std::clamp(std::atoi(val.c_str()), 0, 14)); }
...
"  \033[1m--center-sensitivity <f>\033[0m --control-window  \033[1m--lighter-color\033[0m\n"
  ...
  9. MSAA resolve (if enabled): blit from multisample FBO to screen.
   ...
   10. glfwSwapBuffers presents the frame.
        ...
        if (arg == "--capture" || arg == "--capture-mode") {
            if (++i >= argc) { std::cerr << "--capture requires mic/system/both\n"; return 1; }
    ...
    if (tuiOnly) {
        ...
        auto tuiReload = [&]() {
            ...
            while (std::getline(f, line)) {
                ...
                std::getline(iss >> std::ws, val);
                const char* v = val.empty() ? nullptr : val.c_str();
                if (tok == "--mode" && v) tuiMode = static_cast<VisMode>(std::max(0, std::min(14, std::atoi(v))));
        ...
        auto adjust_param = [&](int idx, int delta) {
            ...
            switch (idx) {
                case 0: tuiMode = static_cast<VisMode>((static_cast<int>(tuiMode) + delta + 15) % 15); break;
        ...
        auto toggle_param = [&](int idx) {
            switch (idx) {
                case 0: tuiMode = static_cast<VisMode>((static_cast<int>(tuiMode) + 1) % 15); break;
        ...
        while (tuiRunning) {
            ...
            if (now - lastFrameTime >= 0.1) {
                ...
                auto mn = [](VisMode m) {
                    ...
                    case VisMode::ParticleField: return "Particle Field"; case VisMode::LedBars: return "LED Bars";
                ...
                if (preview_h > 2) {
                    ...
                    if (tuiMode == VisMode::TrueXY || tuiMode == VisMode::Lissajous) {
                        int cx_val = preview_w / 2;
                        int cy_val = preview_h / 2;
                        double scale_y = cy_val * 0.8;
                        double scale_x = scale_y * 2.3;
                        double a = 3.0, b = 2.0;
                        double phase = now * 2.5;
                        for (double t = 0; t < 6.28; t += 0.05) {
                            double lx = std::sin(a * t + phase) * scale_x;
                            double ly = std::cos(b * t) * scale_y;
                            int sx = cx_val + lx;
                            int sy = cy_val + ly;
                            renderer.draw_cell(px + sx, py + sy, "█", TuiRendererLocal::CYAN);
                        }
                    } else if (tuiMode == VisMode::Oscilloscope || tuiMode == VisMode::MirroredWaveform) {
                        int center_y = preview_h / 2;
                        for (int col = 0; col < preview_w; ++col) {
                            double pct = (double)col / preview_w;
                            double val = std::sin(pct * 12.0 - now * 7.0) * std::cos(pct * 4.0) * tuiState.zoom;
                            int offset_y = val * (preview_h / 2 - 1);
                            int sy = center_y + offset_y;
                            Color w_color = TuiRendererLocal::interpolate(TuiRendererLocal::TEAL, TuiRendererLocal::MAGENTA, pct);
                            renderer.draw_cell(px + col, py + sy, "█", w_color);
                            if (tuiMode == VisMode::MirroredWaveform) {
                                renderer.draw_cell(px + col, py + center_y - offset_y, "█", w_color);
                            }
                        }
                    } else if (tuiMode == VisMode::SpectrumBars || tuiMode == VisMode::DenseSpectrum || tuiMode == VisMode::CircularSpectrumFilled) {
                        for (int col = 0; col < preview_w; ++col) {
                            double pct = (double)col / preview_w;
                            double base_h = std::exp(-pct * 2.0) * 0.7;
                            double peak1 = std::max(0.0, std::sin(pct * 6.28 - now * 3.5)) * 0.35 * (0.5 + 0.5 * std::sin(now * 5.0));
                            double peak2 = std::max(0.0, std::cos(pct * 14.0 + now * 2.5)) * 0.15;
                            double h_val = (base_h + peak1 + peak2) * tuiState.sensitivity * preview_h;
                            int bar_h = (int)std::clamp(h_val, 0.0, (double)preview_h);
                            for (int r = 0; r < bar_h; ++r) {
                                int sy = preview_h - 1 - r;
                                Color bar_color = TuiRendererLocal::interpolate(TuiRendererLocal::GREEN, TuiRendererLocal::RED, (double)r / preview_h);
                                renderer.draw_cell(px + col, py + sy, "█", bar_color);
                            }
                        }
                    } else {
                        int cx_val = preview_w / 2;
                        int cy_val = preview_h / 2;
                        double scale_y = cy_val * 0.7;
                        double scale_x = scale_y * 2.2;
                        double phase = now * 1.5;
                        for (double angle = 0; angle < 6.28; angle += 0.06) {
                            double rad_mod = 1.0 + 0.3 * std::sin(angle * 6.0 - now * 4.0) * (0.5 + 0.5 * std::cos(now * 3.0));
                            double r_x = scale_x * rad_mod;
                            double r_y = scale_y * rad_mod;
                            int sx = cx_val + std::cos(angle + phase) * r_x;
                            int sy = cy_val + std::sin(angle + phase) * r_y;
                            Color ring_color = TuiRendererLocal::interpolate(TuiRendererLocal::CYAN, TuiRendererLocal::MAGENTA, std::sin(angle) * 0.5 + 0.5);
                            renderer.draw_cell(px + sx, py + sy, "█", ring_color);
                        }
                    }
                }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int k = 0; k <= 9; ++k) {
            ...
            if (glfwGetKey(window, key) == GLFW_PRESS) {
                // SHIFT+1/2/7/8 mapped to modes 12/13/10/11 below
                if ((k == 1 || k == 2 || k == 7 || k == 8) && shiftHeld) continue;
        ...
        if (!output.fillVertices.empty() && state.bloom) {
            const float shadowOff = state.bloomSize * (0.5f + 0.3f * static_cast<float>(state.bloomSteps));
            output.shadowVertices.reserve(output.fillVertices.size());
            for (size_t vi = 0; vi < output.fillVertices.size(); vi += 6) {
                float sx = output.fillVertices[vi] + shadowOff;
                float sy = output.fillVertices[vi + 1] - shadowOff;
        ...
        // --- ImGui control panel ---
        {
            ...
            if (controlWindow) {
                ...
                {
                    Color3 decoColor = state.gradientColors.empty()
                        ? state.randomSolid
                ...
                ImGui::SliderFloat("Zoom", &state.zoom, 0.3f, 5.0f);
                ...
                if (ImGui::CollapsingHeader("Values", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    ImGui::Text("simSpeed: %.2f  bloomShape: %s  centerSens: %.2f",
                        state.simSpeed,
        ...
        if (tuiEnabled) {
            ...
            if (now - lastTuiFrame >= 0.2) {
                ...
                auto inputLabel = [](InputMode m) -> const char* {
                    switch (m) { case InputMode::Microphone: return "Mic"; case InputMode::SystemAudio: return "Sys"; case InputMode::Both: return "Both"; } return "?";
                };

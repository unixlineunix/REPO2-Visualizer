...
std::pair<size_t, size_t> logBinRange(size_t numBins, size_t numGroups, size_t groupIndex) {
    ...
    constexpr float freqLow    = 30.0f;
    constexpr float freqHigh   = 18000.0f;
    const float binLow  = freqLow  * fftSize / sampleRate; // ~2.56
    const float binHigh = freqHigh * fftSize / sampleRate; // ~1536
    const float logLow  = std::log10(binLow + 1.0f);
    const float logHigh = std::log10(binHigh + 1.0f);
    const float t0 = static_cast<float>(groupIndex)      / static_cast<float>(numGroups);
    const float t1 = static_cast<float>(groupIndex + 1)  / static_cast<float>(numGroups);
    const size_t binStart = static_cast<size_t>(std::pow(10.0f, logLow + t0 * (logHigh - logLow)) - 1.0f);
    const size_t binEnd   = static_cast<size_t>(std::pow(10.0f, logLow + t1 * (logHigh - logLow)) - 1.0f);
    size_t bs = std::min(binStart, numBins - 1);
    size_t be = std::clamp(binEnd, bs + 1, numBins);
    return {bs, be};
}
    ...
    for (size_t bar = 0; bar < numBars; ++bar) {
        ...
        if (state.bloom) {
            ...
            if (state.bloomSteps > 0) {
                const float xc = (x0 + x1) * 0.5f;
                for (int si = 0; si < state.bloomSteps; ++si) {
                    const float stepT = static_cast<float>(si + 1) / static_cast<float>(state.bloomSteps);
    ...
    for (size_t col = 0; col < cols; ++col) {
        ...
        for (size_t row = 0; row < rows; ++row) {
            ...
            if (normalizedRow <= level) {
                const Color3 c = resolveColor(state, Color3{1.0f, 1.0f, 1.0f}, normalizedRow);
                r = c.r; g = c.g; b = c.b;
                psize = state.particleSize;
            } else {
                r = 0.08f; g = 0.09f; b = 0.12f;
                psize = state.particleSize * 0.45f;
            }

            pushVertex(out.lineVertices, x, y, r, g, b, psize);
...
ModeOutput buildMirroredDenseSpectrum(const std::vector<float>& magnitudes, std::vector<float>& smoothedHeights, const VisState& state) {
    ...
    for (size_t bar = 0; bar < numBars; ++bar) {
        ...
        // Line tips
        pushVertex(out.lineVertices, xc, yExtent,  topColor.r, topColor.g, topColor.b, 1.0f);
        pushVertex(out.lineVertices, xc, -yExtent, topColor.r, topColor.g, topColor.b, 1.0f);
...
ModeOutput buildCircularMesh(const std::vector<float>& magnitudes, std::vector<float>& smoothedHeights, const VisState& state) {
    ...
    for (size_t i = 0; i < numBars; ++i) {
        ...
        const auto [binStart, binEnd] = logBinRange(numBins, numBars, i);
        const float mag = groupMagnitude(magnitudes, binStart, binEnd);
    ...
    if (state.bloom && state.bloomSteps > 0) {
        out.glowVertices.reserve(numBars * 6 * 6 * static_cast<size_t>(state.bloomSteps) + 144);
        for (int si = 0; si < state.bloomSteps; ++si) {
            const float stepT = static_cast<float>(si + 1) / static_cast<float>(state.bloomSteps);

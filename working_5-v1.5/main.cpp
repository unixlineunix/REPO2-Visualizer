...
int main(int argc, char* argv[]) {
    ...
    auto parseConfigFile = [&](const std::string& path) {
        ...
        while (std::getline(file, line)) {
            ...
            else if (key == "system") { if (v) tryBool(v, startSystem); else startSystem = true; }
            else if (key == "mic") { if (v) tryBool(v, startMic); else startMic = true; }
    ...
    auto loadLiveConfigFile = [](const std::string& path, VisState& state, VisMode& mode) {
        ...
        while (std::getline(file, line)) {
            ...
            if (key == "mode") { mode = static_cast<VisMode>(std::clamp(std::atoi(val.c_str()), 0, 9)); }
...
"  \033[1m--particles <n>\033[0m         --system\033[0m               --control-window \033[1m--lighter-color\033[0m\n"
   ...
   10. glfwSwapBuffers presents the frame.
    ...
    if (tuiOnly) {
        ...
        auto adjust_param = [&](int idx, int delta) {
            ...
            switch (idx) {
                case 0: tuiMode = static_cast<VisMode>((static_cast<int>(tuiMode) + delta + 10) % 10); break;
        ...
        while (tuiRunning) {
            ...
            if (now - lastFrameTime >= 0.1) {
                ...
                auto mn = [](VisMode m) {
                    ...
                    case VisMode::CircularSpectrumFilled: return "Circular Fill"; case VisMode::PulseRings: return "Pulse Rings"; } return "?";
                };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // --- ImGui control panel ---
        {
            ...
            if (controlWindow) {
                ...
                const char* modes[] = {"TrueXY","Oscilloscope","SpectrumBars","MirroredWF",
                                       ...
                                       "CircSpecFilled","PulseRings"};
                int curMode = static_cast<int>(mode);
                if (ImGui::Combo("Mode", &curMode, modes, 10)) {
                    mode = static_cast<VisMode>(curMode);
                }

                { int v = static_cast<int>(state.numBars); if (ImGui::SliderInt("Bars", &v, static_cast<int>(state.minBars), static_cast<int>(state.maxBars))) { state.numBars = static_cast<size_t>(v); resizeBarVectors(state, spectrumHeights, circularSpectrumHeights, denseHeights, circularFilledHeights); } }
                ...
                ImGui::SameLine();
                ImGui::SliderFloat("Intensity", &state.bloomIntensity, 0.5f, 2.5f);
        ...
        if (tuiEnabled) {
            ...
            if (now - lastTuiFrame >= 0.2) {
                ...
                std::cout << "\033[1;30m T=hide  C=control-panel  0-9=mode  Esc=exit\033[0m\n";
                std::cout << "\033[u";
                std::cout.flush();
            }
        }
